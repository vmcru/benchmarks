...
def from_static(x, edges, count=0, transform=None):
    ...
    '''
    #prints: 
    #print(" starting from_statis to produce batch")
...
class GCN(torch.nn.Module):
    def __init__(self, 
        ...
        if test == 1:
            ...
            self.model = TSNet(in_channels=num_node_features, 
                                    ...
                                    #dropout=dropout,
                                    #act=activation_type,
                                    trainable_laziness=False)  # Assuming `deg` is a tensor or replace with actual degree tensor
        else:
    ...
    def forward(self, batch):
        ...
        if self.test == 5:
            newbatch.edge_index = newbatch.edge_index.to(x.device)
            #print(newbatch.batch.shape, x.shape)
            x, _ = self.model(newbatch)
