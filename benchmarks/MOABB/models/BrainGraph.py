...
class GCN(torch.nn.Module):
    def __init__(self, kdeg1, kdeg2, kdeg3, hidden_channels1, hidden_channels2, hidden_channels3, num_node_features, num_classes, channels, activation_type="elu", test=0):
        ...
        if test==1:
            self.test = test
            self.model = models.GCN(in_channels=num_node_features, hidden_channels=hidden_channels1, out_channels=hidden_channels3,num_layers=4)
        elif test==2:
            self.test = test
            self.model = models.GraphSAGE(in_channels=num_node_features, hidden_channels=hidden_channels1, out_channels=hidden_channels3,num_layers=2)
        elif test==3:
            self.text = test
            self.model = models.GAT(in_channels=num_node_features, hidden_channels=hidden_channels1, out_channels=hidden_channels3,num_layers=2)
        else:    
